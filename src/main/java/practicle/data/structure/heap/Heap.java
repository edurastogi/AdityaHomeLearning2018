    ...
    private void trickleDown(int idx) {
        ...
        while (idx < currentSize / 2) {
            ...
            if (top.getKey() >= heapArray[largerChildIdx].getKey()) {
                //successfully ade root the largest key
                break;
            }
   ...
   /* public void displayHeap(){
        ...
        while(true) {
            if (leftIdx <= currentSize) {
                System.out.println(heapArray[leftIdx].getKey());
            }

            if (rightIdx <= currentSize) {
                System.out.println(heapArray[rightIdx].getKey());
            }

            leftIdx = (leftIdx * 2) + 1;
            rightIdx = (rightIdx * 2) + 1;
            if(leftIdx>currentSize && rightIdx>currentSize) break;
        }
    }*/

    private void displayHeapRec(int idx){
        if(idx >=currentSize) return;
        System.out.println(heapArray[idx].getKey());
        displayHeapRec((idx*2)+1);
        displayHeapRec((idx*2)+2);
    }
